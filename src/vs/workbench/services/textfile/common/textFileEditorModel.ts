			...
			if (this.isResolved() && !options.skipSaveParticipants) {
				try {
					...
					if (!saveCancellation.token.isCancellationRequested) {
						...
						try {
							await this.textFileService.files.runSaveParticipants(this, { reason: options.reason ?? SaveReason.EXPLICIT, source: options.sourceResource }, saveCancellation.token);
						} finally {
